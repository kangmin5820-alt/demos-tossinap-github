...
-- 기존 INSERT 정책 삭제
DROP POLICY IF EXISTS "로그인한 사용자가 게시물 작성 가능" ON public.posts;

-- 새로운 INSERT 정책: 공식 게시물은 user_id 없이 가능, 일반 게시물은 로그인 필요
CREATE POLICY "게시물 작성 정책"
ON public.posts FOR INSERT
TO authenticated
WITH CHECK (
  (type = 'official' AND is_official = true) OR 
  (type = 'user' AND auth.uid() = user_id)
);
